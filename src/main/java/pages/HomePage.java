...
public class HomePage {
	...
	public int obterQuantidadeProdutosCarrinho() {
		String qtde = driver.findElement(textoProdutosNoCarrinho).getText();
		qtde = qtde.replace("(", "");
		qtde = qtde.replace(")", "");
		int qtdNoCarrinho = Integer.parseInt(qtde);
		return qtdNoCarrinho;
	}

	public String obterNomeProdutoIndice(int indice) {

		return driver.findElements(descricoesDosProdutos).get(indice).getText();

	}

	public String obterPrecoProdutoIndice(int indice) {
	...
	/**
	 * 
	 * @return retorna o valor negado do usuario logado
	 */
